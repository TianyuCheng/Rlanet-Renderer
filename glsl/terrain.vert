...
vec3 computeNormal(vec2 gridPos) {
    ...
    px.y = texture(heightmap, uvx).x * 1400.0 - 700;
    py.y = texture(heightmap, uvy).x * 1400.0 - 700;
    p.y = texture(heightmap, uv).x * 1400.0 - 700;
...
void main()
{
    ...
    morphedPos.y = texture(heightmap, uv).x * 1400.0 - 700;
